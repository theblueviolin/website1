...
function showLoginForm(error = "") {
    modalContent.innerHTML = `<h3>Login</h3>${error ? `<p style="color:red;font-size:12px;">${error}</p>` : ''}<input type="text" id="l-name" placeholder="User"><input type="password" id="l-pass" placeholder="Pass"><button onclick="handleLogin()">Login</button><span class="modal-link" onclick="showRegisterForm()" style="cursor:pointer;font-size:12px;color:#888;text-decoration:underline;">Create Account</span>`;
}

function showRegisterForm(error = "") {
    modalContent.innerHTML = `<h3>Register</h3>${error ? `<p style="color:red;font-size:12px;">${error}</p>` : ''}<input type="text" id="r-name" placeholder="User"><input type="password" id="r-pass" placeholder="Pass"><input type="password" id="r-conf" placeholder="Confirm"><button onclick="handleRegister()">Create</button><span class="modal-link" onclick="showLoginForm()" style="cursor:pointer;font-size:12px;color:#888;text-decoration:underline;">Back</span>`;
}
...
async function handleRegister() {
    ...
    if (p !== c) return showRegisterForm("No match");
...
function updateAuthUI() { 
    authBtn.innerText = userAccount ? "Logout" : "Login"; 
    welcomeMsg.innerText = userAccount ? `Hi, ${userAccount.username}` : ""; 
}
...
function buildMonthButtons() {
    ...
    for (let i = 0; i < 12; i++) {
        ...
        const b = document.createElement('button'); b.textContent = monthNames[i];
...
async function submitExpense() {
    ...
    const name = document.getElementById('name').value || 'Item';
...
async function loadMonth(key, btn) {
    ...
    if (!userAccount) { rowsEl.innerHTML = '<tr><td colspan="6" style="text-align:center;">Login to view</td></tr>'; return; }
    ...
    rowsEl.innerHTML = filtered.length ? '' : '<tr><td colspan="6" style="text-align:center;">Empty</td></tr>';
    filtered.forEach(e => {
        const idx = expenses.indexOf(e);
        const tr = document.createElement('tr');
        tr.setAttribute('data-id', idx);
        tr.innerHTML = `
            <td><button class="action-btn edit-btn" onclick='editRow(${idx}, ${JSON.stringify(e)})'>Edit</button></td>
            ...
            <td><button class="action-btn remove-btn" onclick="removeExpense(${idx})">×</button></td>
        `;
...
async function saveRow(index) {
    const dateStr = document.getElementById(`edit-date-${index}`).value;
